...
import { promptUndoRequest, promptTimerToggleRequest } from '../../controller/libs/sweetalert';
...
const ChallengerDraft = ({ send, ...props }) => {
    const { draft, readyCheck, settings, isBlue } = useDraftClient({ type: 'challenger', ...props });
    ...
    const setOn = () => {
        promptTimerToggleRequest(settings.on, () => send({ type: 'TIMER_TOGGLE_REQUEST', content: !settings.on }));
    }
    ...
    return (
        <main className="draft--wrapper">
            <div className="pickban-select--wrapper">
                ...
                <ChampionSelectionDisplay {...draft} select={select} lockin={lockin} undo={undo} multiplayer settings={{
                    setOn,
                    setLimit: () => { },
                    isBlue,
                    type: 'CLIENT',
